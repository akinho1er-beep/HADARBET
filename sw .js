...
self.addEventListener('fetch', (event) => {
  ...
  // Les documents HTML (pages) → Network First : on veut TOUJOURS la dernière
  // version déployée. On ne tombe dans le cache qu'en cas de hors-ligne.
  // (Sinon un Cache First garderait l'ancienne version du frontend après un déploiement.)
  if (event.request.mode === 'navigate' ||
      (event.request.method === 'GET' && event.request.destination === 'document')) {
    event.respondWith(
      fetch(event.request).then((fresh) => {
        ...
        return fresh;
      }).catch(() => caches.match(event.request).then(c => c || caches.match('/betting-analyzer.html')))
    );
    return;
  }

  // Tout le reste (JS, CSS, images) → Cache First, puis réseau
  event.respondWith(
